...
};       /*LivingRoom[],Kitchen[],StoreRoom[],EmergencyDepot[],Gym[],Mall[]*/
...
Enemy BossZombie = {"ACID ZOMBIE",100,20};
...
//i. OBJECT INITIALISATION
      ...
      Obj books1021("BOOKS","BOOKS ON THE SHELF","Read Contents.","Books of a medical practitioner.",1021,1,0,0,0);
...
//FIRST PERSON GLOBAL PARAMETERS
	 ...
	 Obj slotobj[2] = {{"hand is free","empty","empty","empty",0,0,0.0,0.0,1.0,0},{"hand is free","empty","empty","empty",0,0,0.0,0.0,1.0,0});
...
cout<<"Zombie type:    "<<Zombie[z].btype<<endl;
cout<<"Zombie health : "<<Zombie[z].zhealth<<endl;
cout<<"Zombie attack : "<<Zombie[z].zattack<<endl<<"<PRESS ENTER>"<<endl;
getch();

while(Zombie[z].zhealth>0 && health>0)
{
cout<<"Press 1 to attack with whatever you got.\nPress 2 to try to dodge its attacks."<<endl;
cin>>ch;
switch(ch)
...
Zombie[z].zhealth-=(slotobj[p].attack + ((alertflag>0)?-1:0));
randomize();
if(random(2)==0) health-=(Zombie[z].zattack/(2+armourobj.armour)); cout<<"Your Health: "<<((health>0)?health:0)<<"  Zombie's health: "<<(((Zombie[z].zhealth)>0)?(Zombie[z].zhealth):0)<<endl;
break;

case '2' :

randomize(); if(random(3)==0) health-=((Zombie[z].zattack)/armourobj.armour);
cout<<"Your Health: "<<((health>0)?health:0)<<"  Zombie's health: "<<(((Zombie[z].zhealth)>0)?(Zombie[z].zhealth):0)<<endl;
break;
...
health-=((Zombie[z].zattack)/armourobj.armour);
break;
}
}
