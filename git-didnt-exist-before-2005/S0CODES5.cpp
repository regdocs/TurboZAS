...
void putdata()
{
	std::cout<<"PROFILE: "<<pfname<<endl;
	std::cout<<"TOTAL ZOMBIES KILLED: "<<zcount<<endl;
	std::cout<<"HIGHEST SCORE: "<<hghscr<<endl;
	std::cout<<"LAST SCORE: "<<lstscr<<endl;
...
srand(time(0)); p = rand()%2; cout<<"Performing attack with <"<<slotobj[p].objname<<":"<<(((slotobj[p].health)>0)?slotobj[p].health:0)<<"/"<<slotobj[p].maxammo<<">\n";
cout<<"\n<PRESS ENTER>"<<endl; getch();
(slotobj[p].health)--;
...
srand(time(0));
if(rand()%2==0) health-=(Zombie[q].zattack/(2+armourobj.armour)); cout<<"Your Health: "<<((health>0)?health:0)<<"  Zombie's health: "<<(((Zombie[q].zhealth)>0)?(Zombie[q].zhealth):0)<<endl;
break;

case '2' :

srand(time(0)); if(rand()%3==0) health-=((Zombie[q].zattack)/armourobj.armour);
...
void titlebox() { system("CLS"); cout<<"***********************ZOMBIE APOCALYPSE SURVIVOR ver1.1************************"<<endl<<endl; }
